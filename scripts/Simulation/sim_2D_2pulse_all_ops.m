function [spectrum, f1, f2, fid, t1, t2, all_ops] = sim_2D_2pulse_all_ops(j_couplings, spins, enriched, pulse_length_1, pulse_length_2, bias_field, r)
% Simulates the 2D Zero Field "COSY" spectrum of a molecule. All operators
%
...
%   all
%
% pulse_length_1 = how long the first pulse should be, in units of hydrogen pi
%   pulses. so, for example a pi/2 hydrogen pulse should be passed 0.5.
%   If omitted, this defaults to 4.
%
% pulse_length_2 = how long the second pulse should be
%
...
%
% Usage:
% [spectrum, f1, f2, fid, t1, t2] = sim_2D_2pulse(j_couplings, spins, enriched, pulse_length_1, pulse_length_2, bias_field, r);
...
if nargin < 6 || numel(bias_field) ~= 3
    bias_field = [0 0 0];
end
...
pulse_length_1 = pulse_length_1*pi_pulse; % Put this in the right units
pulse_length_2 = pulse_length_2*pi_pulse; % Put this also in the right units
...
np1 = 1024; % Number of points in the direct dimension
np2 = 1024; % Number of points in the indirect dimension
...
pulse_1 = expm(-1i*h_pulse*pulse_length_1); % First pulse propagator
pulse_2 = expm(-1i*h_pulse*pulse_length_2); % Second pulse propagator
...
for n=1:ns
  if spins(n) == 0
    rho = rho + h_frac*z_matrices(:, :, n);
  elseif spins(n) == 1
    rho = rho + c_frac*z_matrices(:, :, n);
  elseif spins(n) == 2
    rho = rho + n_frac*z_matrices(:, :, n);
  elseif spins(n) == 3
    rho = rho + p_frac*z_matrices(:, :, n);
  elseif spins(n) == 4
    rho = rho + f_frac*z_matrices(:, :, n);
  end
end

% Start off by applying the pulse to rho
rho2 = pulse_1*rho*pulse_1';
...
all_ops = zeros(2^ns, 2^ns, np1, np2); % Pre allocate the all matrix output
for m = 1:np2
    rho2 = zf2*rho2*zf2'; % Evolve for interval t in the indirect dimension
    
    rho1 = pulse_2*rho2*pulse_2'; % Starting from here, give us our second pulse
