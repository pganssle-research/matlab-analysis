function [out, times, spans] = get_lev(start, freq, num_windows, sr, window, data, asym)
    ...
    %
    % Usage:
    % [out, times, spans] = get_mag(start, freq, num_windows, sr, window, data, asym);
	...
	% Now we just go through and turn it into the output vector
	j = 0;
	while j < indir_points
		% This is a weird way to do this.
		j = j+1;
		
		avg_vec = zeros(num_windows, 1);
