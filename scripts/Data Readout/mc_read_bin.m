...
if(~exist('path', 'var') || isnumeric(path) || ~exist(path, 'file'))  
    ...
    [filepath,filefolder]=uigetfile({'*.mcd;*.pp';'*.pp';'*.mcd'},'Select a binary file', default_dir);
...
FS_CONTAINER = 32; % If it's a container, we should call this recursively and get all the sub-structs.
if(~subs || fs.type ~= FS_CONTAINER)
    ...
    while(~feof(f) && dpos-ipos < fs.size)
        [data, n, fs_buff] = read_fsave_from_file(f, subs); %#ok
...
if(ns >= 1)
    name = fread(f, ns, '*char'); % Not sure if null terminations are necessary, we'll see, I suppose.
    name = deblank(name');
else
    name = '';
end
